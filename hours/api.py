...
def parse_date(date_string: str, end_date: bool = False) -> date:
    ...
    if match:
        ...
        if match.group(3) == "m":
            ...
            day_to_return = (
                monthrange(year_to_return, month_to_return)[1] if end_date else 1
            )
...
#     # TODO: Currently no easy way of doing this in django, get_queryset doesn't
#     #       exist :(
...
class PeriodFilterSet(django_filters.FilterSet):
    target = django_filters.ModelChoiceFilter(
        queryset=Target.objects.all(), widget=TextInput()
    )
