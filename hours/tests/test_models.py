...
def test_get_biweekly_openings_for_range(target, long_period, period_first_week_opening, period_second_week_opening):
    ...
    assert not Opening.objects.filter(
        daily_hours__target=target, daily_hours__date=long_period.period.lower + timedelta(days=1))
    ...
    assert second_week_opening in Opening.objects.filter(
        daily_hours__target=target, daily_hours__date=long_period.period.lower + timedelta(days=7))
    assert not openings[long_period.period.lower + timedelta(days=8)]
    assert not Opening.objects.filter(
        daily_hours__target=target, daily_hours__date=long_period.period.lower + timedelta(days=8))
    assert first_week_opening in openings[long_period.period.lower + timedelta(days=14)]
    assert first_week_opening in Opening.objects.filter(
        daily_hours__target=target, daily_hours__date=long_period.period.lower + timedelta(days=14))
    assert not openings[long_period.period.lower + timedelta(days=15)]
    assert not Opening.objects.filter(
        daily_hours__target=target, daily_hours__date=long_period.period.lower + timedelta(days=15))
